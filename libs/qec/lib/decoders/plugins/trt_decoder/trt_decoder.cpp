...
/// - "engine_load_path": Path to pre-built TensorRT engine file (loads directly)
...
/// - "memory_workspace": Memory workspace size in bytes (optional, default: 1GB)
/// 
/// Note: Only one of onnx_load_path or engine_load_path should be specified, not both.
namespace cudaq::qec {
...
public:
  ...
  trt_decoder(const cudaqx::tensor<uint8_t> &H,
    ...
    try {
        ...
        if (has_engine_path) {
            ...
            engine_ = std::unique_ptr<nvinfer1::ICudaEngine>(runtime->deserializeCudaEngine(engineData.data(), engineData.size()));
            if (!engine_) {
            ...
            engine_ = trt_decoder_internal::build_engine_from_onnx(onnx_model_path, params, gLogger);
            
            // Save engine if requested
            if (params.contains("engine_save_path")) {
                std::string engine_save_path = params.get<std::string>("engine_save_path");
                trt_decoder_internal::save_engine_to_file(engine_.get(), engine_save_path);
            }
        }
        ...
        auto inputDims = engine_->getTensorShape(engine_->getIOTensorName(input_index_));
        input_size_ = 1;
        for (int j = 0; j < inputDims.nbDims; ++j) input_size_ *= inputDims.d[j];

        auto outputDims = engine_->getTensorShape(engine_->getIOTensorName(output_index_));
        output_size_ = 1;
        for (int j = 0; j < outputDims.nbDims; ++j) output_size_ *= outputDims.d[j];
  ...
  virtual decoder_result decode(const std::vector<float_t> &syndrome) override {
    ...
    try {
        ...
        for (std::size_t i = 0; i < syndrome.size() && i < static_cast<std::size_t>(input_size_); i++) {
            input_host[i] = static_cast<float>(syndrome[i]);
        }

        // Copy input to GPU
        cudaMemcpy(buffers_[input_index_], input_host.data(), input_size_ * sizeof(float), cudaMemcpyHostToDevice);

        // Set tensor addresses for TensorRT V1 API
        context_->setTensorAddress(engine_->getIOTensorName(input_index_), buffers_[input_index_]);
        context_->setTensorAddress(engine_->getIOTensorName(output_index_), buffers_[output_index_]);
        ...
        cudaMemcpy(output_host.data(), buffers_[output_index_], output_size_ * sizeof(float), cudaMemcpyDeviceToHost);

        // Postprocess output to get error probabilities
        // Map output to block_size (number of qubits)
        result.result.resize(block_size);
        for (std::size_t i = 0; i < block_size && i < static_cast<std::size_t>(output_size_); i++) {
  ...
  virtual ~trt_decoder() {
    ...
    if (initialized_) {
        ...
        // TensorRT engine and context will be automatically destroyed by unique_ptr
    }
  }
...
void validate_trt_decoder_parameters(const cudaqx::heterogeneous_map& params) {
    ...
    if (has_onnx_path && has_engine_path) {
        throw std::runtime_error("TensorRT decoder cannot specify both 'onnx_load_path' and 'engine_load_path' parameters. Please choose one.");
    }
    
    if (!has_onnx_path && !has_engine_path) {
        throw std::runtime_error("TensorRT decoder requires either 'onnx_load_path' or 'engine_load_path' parameter");
    }
}

// Implementation of the helper function to parse and configure precision settings
void parse_precision(const std::string& precision, nvinfer1::IBuilderConfig* config) {
    ...
    if (precision == "fp16") {
        if (platform.device_has_fp16()) {
            ...
            std::cerr << "Warning: FP16 requested but not supported on this platform, using FP32" << std::endl;
        }
    } else if (precision == "bf16") {
        if (platform.device_has_bf16()) {
            config->setFlag(nvinfer1::BuilderFlag::kBF16);
        } else {
            std::cerr << "Warning: BF16 requested but not supported on this platform, using FP32" << std::endl;
        }
    } else if (precision == "int8") {
        if (platform.device_has_int8()) {
            config->setFlag(nvinfer1::BuilderFlag::kINT8);
        } else {
            std::cerr << "Warning: INT8 requested but not supported on this platform, using FP32" << std::endl;
        }
    } else if (precision == "fp8") {
        if (platform.device_has_fp8()) {
            config->setFlag(nvinfer1::BuilderFlag::kFP8);
        } else {
            std::cerr << "Warning: FP8 requested but not supported on this platform, using FP32" << std::endl;
        }
    } else if (precision == "tf32") {
        if (platform.device_has_tf32()) {
            config->setFlag(nvinfer1::BuilderFlag::kTF32);
        } else {
            std::cerr << "Warning: TF32 requested but not supported on this platform, using FP32" << std::endl;
        }
        ...
        // TF32 is controlled by the kTF32 flag, which is enabled by default in newer TensorRT versions
        ...
        std::cerr << "Warning: Unknown precision '" << precision << "', using default (best)" << std::endl;
    }
}

std::unique_ptr<nvinfer1::ICudaEngine> build_engine_from_onnx(const std::string& onnx_model_path, const cudaqx::heterogeneous_map& params, nvinfer1::ILogger& logger) {
...
void save_engine_to_file(nvinfer1::ICudaEngine* engine, const std::string& file_path) {
    if (!engine) {
