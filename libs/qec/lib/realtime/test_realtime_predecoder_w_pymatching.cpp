...
/// Shared state for the PyMatching decode worker pool.  Owns a pool of
/// decoder instances (one per worker thread) and accumulates timing /
/// syndrome-density statistics across all workers for the final report.
struct DecoderContext {
  /// Pre-built PyMatching decoder instances.  Each worker thread claims
  /// exactly one via acquire_decoder() and reuses it for the lifetime of
  /// the run; the pool is sized to match the number of decode workers.
  std::vector<std::unique_ptr<cudaq::qec::decoder>> decoders;
  std::atomic<int> next_decoder_idx{0};

  // Surface-code geometry used to reshape the flat residual detector
  // vector into per-round slices when decoding with per-slice H.
  int z_stabilizers = 0;
  int spatial_slices = 0;
  int num_residual_detectors = 0;

  /// When true, decode against the full (all-rounds) parity-check matrix
  /// loaded from --data-dir.  When false, fall back to the per-round
  /// surface-code H_z generated by cudaq-qec.
  bool use_full_H = false;

  /// Return a thread-local decoder from the pool.  Each calling thread
  /// is permanently assigned an index on first call (via thread_local),
  /// avoiding contention on the atomic after the initial claim.
  ...
  // Aggregate timing stats (updated atomically by each decode worker).
  std::atomic<int64_t> total_decode_us{0};
  std::atomic<int64_t> total_worker_us{0};
  std::atomic<int> decode_count{0};

  // Syndrome density tracking: count of nonzero detectors before
  // (input) and after (output) the AI predecoder, summed across all
  // requests.  Used to report the density reduction percentage.
...
/// Per-worker context for the pre_launch_fn DMA callback.  The dispatcher
/// calls this on the CPU just before cudaGraphLaunch to copy syndrome data
/// from the ring buffer slot into the predecoder's TRT input buffer.
struct PreLaunchCopyCtx {
  void *d_trt_input;          ///< Device pointer to this worker's TRT input
  size_t input_size;          ///< Bytes to copy (model input size)
  void **h_ring_ptrs;         ///< Mapped host pointer array (mailbox slot)
  uint8_t *rx_data_dev_base;  ///< Ring buffer device base (mapped allocation)
  uint8_t *rx_data_host_base; ///< Ring buffer host base (pinned)
  // The dispatcher passes a device pointer (slot_dev) derived from
  // rx_data_dev_base.  We convert it back to a host pointer via offset
  // arithmetic and issue an H2D copy, which works for any target GPU
  // since pinned host memory is DMA-accessible from all devices.
};
...
/// Per-worker application state passed to the pipeline's CPU stage callback
/// via CpuStageContext::user_context.  Each predecoder worker owns one of
...
struct WorkerCtx {
  ...
  int max_requests = 0;                   ///< Size of the above arrays
  const uint8_t *obs_row = nullptr;       ///< Ground-truth observable row
  size_t obs_row_size = 0;                ///< Length of obs_row in bytes
};

/// Payload written into the RPC response slot after PyMatching completes.
/// Packed to match the wire format expected by the ring buffer consumer.
struct __attribute__((packed)) DecodeResponse {
  int32_t total_corrections; ///< Number of edges flipped by MWPM
  int32_t converged;         ///< 1 if all decode slices converged, 0 otherwise
};
...
/// Work item handed off from the predecoder GPU stage to the PyMatching
/// decode thread pool.  Carries the ring buffer slot info needed to write
/// the RPC response and signal completion after decoding finishes.
struct PyMatchJob {
  int origin_slot;       ///< Ring buffer slot to complete via tx_flags
  uint64_t request_id;   ///< Monotonic request ID for correctness tracking
  void *ring_buffer_ptr; ///< Host pointer to the slot's RX data area
};

/// Thread-safe MPMC queue connecting predecoder workers (producers) to
/// PyMatching decode workers (consumers).  Predecoder workers enqueue
/// jobs after copying the GPU output; decode workers block in pop()
/// until work arrives or shutdown() is called.
class PyMatchQueue {
public:
  ...
  /// Block until a job is available.  Returns false only after shutdown()
  /// has been called and the queue is drained.
  ...
  /// Signal all waiting consumers to exit after draining remaining jobs.
  void shutdown() {
    {
      std::lock_guard<std::mutex> lk(mtx_);
      stop_ = true;
