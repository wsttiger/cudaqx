...
cudaq::spin_op seeley_richard_love(std::size_t i, std::size_t j,
  ...
  cudaq::spin_op seeley_richard_love_result = cudaq::spin_op::empty();

  // Case 0
  if (i == j) {
    if (not occupation_set(i).empty()) {
      cudaq::spin_op_term ops;
  ...
  // Case 2
    ...
    cudaq::spin_op_term right_pad_1;
    auto P0_minus_alpha =
    ...
    cudaq::spin_op_term right_pad_2;
    auto P2_minus_alpha =
  ...
  // Case 7
    ...
    cudaq::spin_op_term right_pad_3;
    for (auto index :
         set_difference(P2_ij_set(i, j), alpha_set(i, j, n_qubits))) {
      right_pad_3 *= cudaq::spin::z(index);
    }
    cudaq::spin_op_term right_pad_4;
    for (auto index :
  ...
  return seeley_richard_love_result.canonicalize().trim();
}
...
cudaq::spin_op bravyi_kitaev::generate(const double constant,
  ...
  cudaq::spin_op spin_hamiltonian = cudaq::spin_op::empty();
  ...
  return spin_hamiltonian.canonicalize().trim();
}
} // namespace cudaq::solvers
